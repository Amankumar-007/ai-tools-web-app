...
const genericContent = `"use client";
...
routes.forEach(route => {
  const dirPath = path.join(__dirname, 'src', 'app', '(routes)', route);
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }
  fs.writeFileSync(path.join(dirPath, 'loading.tsx'), genericContent);
  console.log('Created generic loading for', route);
});
